...
| R11 | 静态前端连接独立模型后端 | API Key 只在 Python FastAPI 服务端；前端通过可配置地址和受限 CORS 调用 |
| R12 | Agent 控制面 | Evaluation 展示证据索引、Reasoner、Grounding、人类门禁和变更监控状态 |
| R13 | 公共演示成本保护 | 模型端点支持来源限制、每 IP 限流、并发上限和调用超时 |
